...
const CustomMarkdownRenderer = ({ mainState }) => {
    ...
    return (
        <div id="my-markdown-renderer" className='chat-markdown w-full h-full overflow-auto bg-white p-8 prose max-w-none'>
            <ReactMarkdown 
                ...
                components={{
                    code({ node, className, children, ...props }) {
                        ...
                        return match ? (
                            ...
                            <code className={`${className} bg-gray-100 rounded px-1 py-0.5 text-sm text-gray-800`} {...props}>
                                {children}
                            </code>
                        )
                    }
